...
import erc2612Addr from '../../deployments/dev/Etherloop_2612.json'
import erc2612Abi from '../../deployments/abi/Etherloop_2612.json'
  ...
  methods: {
    ...
    readContract() {
      this.provider.getBalance(this.account).then((r) => {
        this.ethbalance = ethers.utils.formatUnits(r, "ether");
      });
      ...
      this.erc20Token.totalSupply().then((r) => {
        this.supply = ethers.utils.formatUnits(r, 2);
      })

      this.erc20Token.balanceOf(this.account).then((r) => {
        this.balance = ethers.utils.formatUnits(r, 2);
      })

      this.bank.getBalance(erc2612Addr.address,this.account).then((r) => {
        this.myDeposit = ethers.utils.formatUnits(r, 2);
      })

    },

    getApproved() {
      this.erc20Token.allowance(this.account, this.bankAddress.address).then((r) => {
        this.approved = ethers.utils.formatUnits(r, 2);
      })
    },
    ...
    transfer() {
      ...
      let amount = ethers.utils.parseUnits(this.amount, 2);
    ...
    async approve() {
      ...
      try {
        let amount = ethers.utils.parseUnits(this.approveAmount, 2);
    ...
    async deposit() {
      ...
      try {
        let amount = ethers.utils.parseUnits(this.approveAmount, 2);
        let tx = await this.bank.deposit(erc2612Addr.address, amount);
        await tx.wait();
        this.getApproved();
        this.readContract();
      }
    ...
    async permitDeposit() {
      ...
      let amount =  ethers.utils.parseUnits(this.stakeAmount,2).toString();
      ...
      try {
        let tx = await this.bank.permitDeposit(erc2612Addr.address, amount, this.deadline, v, r, s);
    ...
    async withdraw() {
      ...
      let amount = await this.bank.getBalance(erc2612Addr.address,this.account);
      let tx = await this.bank.withdraw(erc2612Addr.address,amount);
      await tx.wait();
      this.readContract();
    }
  }
